...
<div class="container-fluid p-0">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h2 class="h3 mb-0"><strong>Modifier</strong> {{ $invoice->invoice_number }}</h2>
        <a href="{{ route('invoicing.show', $invoice) }}" class="btn btn-outline-secondary btn-sm">Retour</a>
    </div>
    ...
    <form action="{{ route('invoicing.update', $invoice) }}" method="POST" id="invoice-form">
        @csrf
        @method('PUT')
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="client_name" class="form-control" required value="{{ old('client_name', $invoice->client_name) }}">
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Contact (téléphone / email)</label>
                            <input type="text" name="client_contact" class="form-control" value="{{ old('client_contact', $invoice->client_contact) }}">
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Adresse</label>
                            <input type="text" name="client_address" class="form-control" value="{{ old('client_address', $invoice->client_address) }}">
                        </div>
                        <div class="mb-3">
                            <label class="form-label">NIF / identifiant fiscal</label>
                            <input type="text" name="client_tax_id" class="form-control" value="{{ old('client_tax_id', $invoice->client_tax_id) }}">
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="card-body">
                        ...
                        <div class="row g-3">
                            <div class="col-6">
                                ...
                                <input type="text" class="form-control" value="{{ $invoice->issue_date->format('d/m/Y') }}" disabled>
                                <small class="text-muted">Non modifiable : le numéro de facture est déjà alloué pour cette date.</small>
                            </div>
                            <div class="col-6">
                                <label class="form-label">Échéance *</label>
                                <input type="date" name="due_date" class="form-control" required value="{{ old('due_date', $invoice->due_date->toDateString()) }}">
                            </div>
                            <div class="col-6">
                                <label class="form-label">TVA (%)</label>
                                <input type="number" step="0.01" min="0" max="100" name="tax_rate" id="tax_rate" class="form-control" value="{{ old('tax_rate', $invoice->tax_rate) }}">
                            </div>
                        </div>
                        <div class="mb-0 mt-3">
...
@php
    $existingItemsForJs = $invoice->items->map(fn ($item) => [
