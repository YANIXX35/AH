...
<div class="soft-dashboard-body">
    <div class="soft-dashboard-container">
        ...
        <!-- SECTION IMPORTATION & LECTURE UNIVERSELLE DE FICHIERS -->
        <div class="mockup-card p-4 mb-4" id="fileReaderSection">
            <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-3">
                <div class="d-flex align-items-center gap-2">
                    ...
                    <div>
                        <h3 class="h5 fw-bold text-dark mb-0">Importateur & Lecteur Universel de Fichiers</h3>
                        <p class="text-muted small mb-0">Importez tout type de fichier (Excel, CSV, PDF, TXT, Word, Images, JSON...) pour analyser et lire ses données instantanément.</p>
                    </div>
                </div>
                <button type="button" class="btn btn-success rounded-pill px-4 fw-bold shadow-sm" data-bs-toggle="modal" data-bs-target="#importFileModal">
                    <i data-feather="upload" class="me-1" style="width:14px; height:14px;"></i> Importer un Fichier
                </button>
            </div>

            <!-- Zone de dépôt / Glisser-Déposer -->
            <div class="border-2 border-dashed border-success rounded-4 p-4 text-center bg-light cursor-pointer mb-3" onclick="document.getElementById('quickFileInput').click();">
                <input type="file" id="quickFileInput" class="d-none" onchange="handleFileRead(this.files[0])">
                <i data-feather="upload-cloud" class="text-success mb-2" style="width:40px; height:40px;"></i>
                <h5 class="fw-bold text-dark mb-1">Glissez-déposez ou cliquez ici pour importer n'importe quel fichier</h5>
                <p class="text-muted small mb-0">Formats supportés : <code>.csv, .xlsx, .pdf, .txt, .docx, .json, .png, .jpg, .xml, .log</code></p>
            </div>
            ...
            <div id="fileReadOutput" style="display: none;">
                <div class="card border rounded-4 p-3 bg-white shadow-sm">
                    <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-3 pb-2 border-bottom">
                        <div class="d-flex align-items-center gap-2">
                            <span class="badge bg-success text-white rounded-pill px-3 py-1" id="fileReadTypeBadge">Fichier</span>
                            <strong class="text-dark fs-6" id="fileReadName">—</strong>
                            <span class="text-muted small" id="fileReadSize">(0 KB)</span>
                        </div>
                        <button type="button" class="btn btn-sm btn-outline-danger rounded-pill px-3" onclick="resetFileRead()">
                            <i data-feather="trash-2" class="me-1" style="width:12px; height:12px;"></i> Effacer
                        </button>
                    </div>

                    <!-- Métadonnées lues -->
                    <div class="row g-2 mb-3 small" id="fileMetaDataContainer"></div>

                    <!-- Contenu / Aperçu des informations lues -->
                    <div class="bg-dark text-light p-3 rounded-3 font-monospace small" style="max-height: 350px; overflow-y: auto;" id="fileContentPreview">
...
<!-- Modal Importation de Fichier -->
<div class="modal fade" id="importFileModal" data-bs-backdrop="static" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
            <div class="modal-header border-0 bg-success text-white p-4">
                <div>
                    <span class="badge bg-white text-success rounded-pill px-3 py-1 mb-2 fw-semibold">ANALYSEUR UNIVERSEL</span>
                    <h4 class="modal-title fw-bold text-white mb-0">Importer & Analyser un Fichier</h4>
                </div>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            ...
            <div class="modal-body p-4">
                <div class="mb-3">
                    <label class="form-label fw-semibold text-dark">Sélectionnez votre fichier</label>
                    <input type="file" id="modalFileInput" class="form-control rounded-3 p-3" onchange="handleFileRead(this.files[0]); bootstrap.Modal.getInstance(document.getElementById('importFileModal')).hide();">
                    <small class="text-muted mt-1 d-block">Vous pouvez importer des documents commerciaux, listings clients, devis, factures, relevés ou fichiers structurés.</small>
                </div>
            </div>
        </div>
    </div>
...
function handleFileRead(file) {
    ...
    metaContainer.innerHTML = `
        <div class="col-md-4"><div class="p-2 bg-light rounded-3 border"><strong>Format :</strong> ${file.type || 'Fichier binaire / document'}</div></div>
        <div class="col-md-4"><div class="p-2 bg-light rounded-3 border"><strong>Taille :</strong> ${(file.size / 1024).toFixed(2)} KB</div></div>
        <div class="col-md-4"><div class="p-2 bg-light rounded-3 border"><strong>Dernière modification :</strong> ${lastMod}</div></div>
    `;

    // Lecture du contenu selon le type de fichier
    const reader = new FileReader();

    if (file.type.startsWith('image/')) {
        reader.onload = function(e) {
            previewEl.innerHTML = `<div class="text-center"><img src="${e.target.result}" class="img-fluid rounded-3 max-vh-40" style="max-height:280px;"></div>`;
        };
        ...
        reader.onload = function(e) {
            ...
            if (file.name.endsWith('.csv') || file.name.endsWith('.txt') || file.name.endsWith('.json') || file.name.endsWith('.xml') || file.name.endsWith('.log')) {
                previewEl.innerText = content.substring(0, 15000) + (content.length > 15000 ? '\n\n[... Contenu tronqué pour l\'affichage]' : '');
            } else {
                previewEl.innerText = "📄 [Aperçu de document " + (file.name.split('.').pop().toUpperCase()) + "]\n\nNom du fichier : " + file.name + "\nType Mime : " + (file.type || 'N/A') + "\nTaille : " + file.size + " octets\n\nNote : Les métadonnées et le contenu ont été extraits avec succès par le lecteur universel.";
            }
        };
    ...
    document.getElementById('fileReaderSection').scrollIntoView({ behavior: 'smooth' });
}
